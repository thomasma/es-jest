...
public class RunMe {
    ...
    public static void main(String[] args) {
        try {
            // Get Jest client
            ClientConfig clientConfig = new ClientConfig.Builder("http://localhost:9200").multiThreaded(true)
                    .build();
    ...
    private static void createTestIndex(final JestClient jestClient) throws Exception {
        ...
        jestClient.execute(new CreateIndex.Builder(DIARY_INDEX_NAME).settings(settings.build().getAsMap())
                .build());
    }
